...
function mergeObjects(objects) {
  return objects.reduce((acc, currObj) => {
    const keyAcc = Object.keys(acc);
    const keyCurrObj = Object.keys(currObj);
    const copyCurrObj = { ...currObj };
    keyCurrObj.forEach((item) => {
      if (keyAcc.includes(item)) {
        copyCurrObj[item] = acc[item] + currObj[item];
      }
    });
    return { ...acc, ...copyCurrObj };
  }, {});
}
...
function removeProperties(obj, keys) {
  ...
  const myObj = { ...obj };
  const myArr = [...keys];
  myArr.forEach((item) => delete myObj[item]);
  return myObj;
}
...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach((item) =>
    item[1].forEach((i) => {
      arrLetters.push(item[0]);
      arrNambers.push(i);
    })
  );

  const result = new Array(arrNambers.length).fill(null);
  for (let i = 0; i < arrNambers.length; i += 1) {
    result[arrNambers[i]] = arrLetters[i];
  }
  return result.join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === 25) money += 25;
    else if (queue[i] === 50 && money >= 25) money += 25;
    else if (queue[i] === 100 && money >= 75) money += 75;
    else return false;
  }
  return true;
}
...
function group(array, keySelector, valueSelector) {
  const result2 = Object.groupBy(array, keySelector);
  const keyResult2 = Object.keys(result2);
  const res = new Map();
  for (let i = 0; i < keyResult2.length; i += 1)
    res.set(keyResult2[i], result2[keyResult2[i]].map(valueSelector));
  return res;
}
